...
## 依存性を確認しよう
Gemの動作要件として、別のGemがバージョン付きで指定されていることがよくあります。  
...
## アップデート順序
Rubyのバージョンを上げる際に、一気にバージョンを上げると危険との情報を頂けましたので、  
私は `2.4.0` → `2.5.0` → `2.6.2` という段階を踏んでアップデートしました。  

`2.4.0` → `2.5.0` へのバージョンアップ成功するが、 
`2.5.0` → `2.6.2` でエラーになるケースがありました。  
